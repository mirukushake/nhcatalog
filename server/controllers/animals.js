...
async function listVillagers (ctx) {
  ...
  const animals = await Animal.query()
    ...
    .join('species_name', 'species_name.species_id', 'speciesInfo.id').where('species_name.lang_id', language)
    ...
    .select('animals.id as id', 'animals.identifier as identifier', 'personality as personality_id', 'personality_names.name as personality', 'birthday', 'species as species_id','species_name.name as species')
    .page(page, size);
